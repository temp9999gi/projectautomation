...
def loadModel(url,metamodel=None, toupdate = None):
		...
		#print '?????????????stmer.objectById', stmer.objectById
		#print 'metamodel',stmer.metamodel
		
		aModelInfo=ModelInfo()
		aModelInfo.setModel(model)
		aModelInfo.setStreamer(stmer)
		#return model
		return aModelInfo
...
##						raise "Model rootOwner"+self.rootOwner+" is not an attribure or a composition:",model
	...
	def findIds(self,node):
		...
		if isinstance(node, xml.dom.minidom.Element):
			...
			#kusung modify ????????????
			#-----------------------------------------------------------
##			if node.nodeName == 'UML:TaggedValue':
				#############print 'node.nodeName', node.nodeName
				###self.extractElement(node)
				#extractElement
            #-----------------------------------------------------------
##			if node.nodeName == 'UML:Interface':
##				print '-----------------------------------------------------------'
##				print 'node.nodeName', node.nodeName

			for c in node.childNodes:
				if c.nodeType != xml.dom.Node.TEXT_NODE: #text 노드가 아니면
					self.findIds(c)
##					print '-----------------------------------------------------------'
##					print 'node', c.nodeName

									
	#얘가 실질적인 XMIContent를 추출한다.
	def extractXMIContent(self,node):
			...
			if node.nodeName=='XMI.content':
				...
				for c in node.childNodes:
					#kusung modify
					#-----------------------------------------------------------
					if c.nodeName in ['UML:TaggedValue']:
						###print 'c.nodeName', c.nodeName
						self.extractElement(c)
						
##					if c.nodeName in ['UML:Interface']:
##						###print 'c.nodeName', c.nodeName
##						self.extractElement(c)
##					if c.nodeName == 'UML:Interface':
##						print '-----------------------------------------------------------'
##						print 'c.nodeName', c.nodeName


					if c.nodeType!= xml.dom.Node.TEXT_NODE and re.search("Model",c.nodeName,re.IGNORECASE):
						if not root: #root에 값이 할당되어 있지 않다면..
	...
	def extractElement(self,node):
		...
		# Is defined?
		if xmi_id:
			if self.objectById.has_key(xmi_id): #a.has_key(k): True if a has a key k, else False
		...
		if self.metamodel.__dict__.has_key(name):
				...
				if issubclass(factory,MMAtom): #issubclass( class, classinfo) Return true if class is a subclass (direct or indirect) of classinfo
					# Note, create it by xmi.uuid!
					obj=apply(factory,(),{'__uniqueID__':xmi_uuid}) #apply(callable_object, args) Call a callable Python object callable_object
			...
			if obj.__mm__[aname][2]==1:
				...
				if v:
					if type(v)==types.UnicodeType:
							...
							v=v.encode('cp949') #kusung modify
