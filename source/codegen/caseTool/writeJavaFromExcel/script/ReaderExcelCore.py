...
# Description:
# Easily extract data from microsoft excel files using this wrapper class for xlrd (http://www.lexicon.net/sjmachin/xlrd.htm). 
# The class allows you to create a generator which returns excel data one row at a time as either a list or dictionary. 
# I found this very handy for easily pulling in a variety of excel files without having to deal with COM calls or 
# even needing to have windows.
# I use this class all the time to simplify excel imports. I hope some people find it useful for them as well.

class ReaderExcelCore(object):
    """ Simple OS Independent Class for Extracting Data from Excel Files 
        ...
        Quick Example:
        xl = ReaderExcelCore('testdata.xls')
    ...
    def __init__(self, filename):
        """ Returns a ReaderExcelCore object of the specified filename - this may
    ...
    def ncols(self, worksheet):
        """ Return the number of fields in a worksheet """
        return self.__sheets__[worksheet]['cols']
    def variables(self,worksheet):
        """ Returns a list of Field Names in the file,
...
#Quick Example:
#
# xl = ReaderExcelCore('input.xls')
# sheetnames = xl.worksheets()
# 
# for sheet in sheetnames:
#     if sheet == 'TABLE':
#         print sheet
#         for a_col, b_col, c_col in xl.getIter(sheet,True):
#             print a_col, b_col, c_col 
